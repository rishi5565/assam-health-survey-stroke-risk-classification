...
def univarstats(df):
    ...
    for col in df:
        if pd.api.types.is_numeric_dtype(df[col]):
            output_df.loc[col] = [df[col].dtype, pd.api.types.is_numeric_dtype(df[col]), df[col].count(), df[col].isnull().sum(), df[col].nunique(), df[col].mode().values[0], np.round(df[col].mean(), 2),
             df[col].min(), df[col].quantile(0.25), df[col].median(), df[col].quantile(0.75), df[col].max(), np.round(df[col].std(), 2), np.round(df[col].skew(), 2), np.round(df[col].kurt(), 2)]
